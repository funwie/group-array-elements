...
def group_array_elements(array, n):
  """
    Group array items into n equal sizes. 
...
for i in range(1, len(input_array_main) + 1):
  grouped_arrays = group_array_elements(input_array_main, i)
  assert len(grouped_arrays) == i

for i in range(1, len(input_array_20) + 1):
  grouped_arrays = group_array_elements(input_array_20, i)
  assert len(grouped_arrays) == i